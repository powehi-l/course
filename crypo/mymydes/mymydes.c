...
static void perm_init(char perm[16][256][8], char p[64]);
static void permute(char *inblock, char perm[16][256][8], char *outblock);
...
// plaintext_32bit_expanded_to_48bit_table
// bytebit
// sbox
// sbox_perm_table
...
//    for(i = 0; i<8;i++){
//       for(j = 0; j < 6; j++){
//          row = (plaintext_32bit_expanded_to_48bit_table[6*i+j]-1)/8;
//          col = (plaintext_32bit_expanded_to_48bit_table[6*i+j]-1)%8;
//          plaintext[i] |= (bytebit[col] && s[row])?bytebit[j+2]:0x00;
//       }
//    }
//    for(i = 0; i<8;i++){
...
//    memset(t,0,4);
//    for(i = 0; i<32; i++){
//       row = (sbox_perm_table[i]-1)/8;
//       col = (sbox_perm_table[i]-1)%8;
//       t[i/8] |= (bytebit[col] && m[row]) ?bytebit[col]:0x00;
//    }
//    cp = (char *)&rval;
//    cp[0] = t[3];
//    cp[1] = t[2];
...
int des_init(int mode){
   ...
   iperm = (char (*)[256][8])malloc(sizeof(char) * 16 * 256 * 8);
   if(iperm == NULL)
   {
      free((char *)sbox_output_perm_table);
      free((char *)kn);
   ...
   fperm = (char (*)[256][8])malloc(sizeof(char) * 16 * 256 * 8);
   if(fperm == NULL)
   {
      free((char *)sbox_output_perm_table);
      free((char *)kn);
