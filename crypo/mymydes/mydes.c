...
void des_encrypt_key(unsigned char *des_cipher, unsigned char *mykey){
   des_init(0);
   des_set_key(mykey);
   des_encrypt((char *)des_cipher);
   des_done();
}
int main()
{
...
void des_cfb_encrypt(unsigned char p[],int n,unsigned char des_seed_iv[],unsigned char des_iv[],unsigned char des_seed_key[]){
   int i,j;
   unsigned char des_final_cipher[0x100], des_key[8], des_cipher[8];
   for(i = 0; i<8; i++){
      //P1 ^ K = C1  =>    K = C1 ^ P1
      //P2 ^ K = C2
      //so C2 = P2 ^ P1 ^ C1
   ...
   for(i = 0; i < n; i++){
      for(j = 0; j < 8; j++){
         des_cipher[i] = des_iv[i];
      }
      des_encrypt_key(des_cipher, des_key);
      des_final_cipher[i] = des_cipher[0] ^ p[i];
      for(j = 0; j<7; j++){
         des_iv[j] = des_iv[j+1];
      }
      des_iv[7] = des_final_cipher[i];
   }
   for(i = 0; i<n; i++){
      printf("%02X", des_final_cipher[i]);
   }
   return;
}