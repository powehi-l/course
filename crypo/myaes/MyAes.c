...
void aes_polynomial_mul(unsigned char x[4], unsigned char y[4], unsigned char z[4]){
   unsigned char m[4], temp;
   int i,j;
   for(i=0; i<4; i++){
      m[i] = 0;
      for(j=0; j<4; j++){
         m[i] ^= (unsigned char)aes_8bit_mul_mod_0x11B(x[(i+j+1)%4], y[j]);
      }
   }
   temp = m[0];
   m[0] = m[2];
   m[2] = temp;
   for(i = 0; i<4; i++) z[i] = m[i];
   return;
}
void ByteSubInverse(unsigned char *p, int n){
   int i;
...
void MixColumnInverse(unsigned char *p, unsigned char a[4], int do_mul){
   int i,j;
   unsigned char temp[4][4], b[4];
   for(i = 0; i<4; i++){
      for(j = 0; j<4; j++) b[j] = p[4*i+j];
      if(do_mul) aes_polynomial_mul(a,b,b);
      for(j=0; j<4; j++) temp[i][j] = b[j];
   }
